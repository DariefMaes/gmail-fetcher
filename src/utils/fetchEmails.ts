...
function extractNameAndEmail(input) {
  ...
  if (match) {
    return {
      name: match[1] || null, // Name might be null if not present
      email: match[2],
    };
  }
...
const cleanEmail = (email: string) => {
  ...
  const unwantedCharsRemoved = crlfRemoved.replace(
    /[\u200B\u0000-\u001F\u007F-\u009F]/g,
    ""
  );
...
const findPlainTextOrHTML = (
  ...
  if (
    ...
    const htmlContent = Buffer.from(emailOrPart.body.data, "base64").toString(
      "utf-8"
    );
  ...
  if (emailOrPart.parts) {
    for (let innerPart of emailOrPart.parts) {
      ...
      if (content) return cleanEmail(content); // Return the first text/plain or text/html found
    }
  }
...
export const fetchEmails = async (data: GoogleApiData) => {
  ...
  if (!latestEmail || !latestEmail.email_date) {
    ...
    query = `-label:SOCIAL -label:PROMOTIONS -label:FORUMS after:${Math.floor(
      oneDayAgo.getTime() / 1000
    )}`;
  } else {
    ...
    console.log(
      "Latest email",
      latestEmail.email_id,
      dateToEpochSeconds(emailDate)
    );
    query = `-label:SOCIAL -label:PROMOTIONS -label:FORUMS after:${dateToEpochSeconds(
      emailDate
    ).toString()}`;
  }
